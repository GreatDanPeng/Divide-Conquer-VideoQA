...
def annotate(predictions, ground_truth_data, output_dir, experiment_scores):
    ...
    for video_id, ground_truth_qa_pairs in ground_truth_data.items():
        ...
        for idx, qa_pair in enumerate(ground_truth_qa_pairs):
            ...
            # Match the corresponding segment (e.g., segment1 for the first question)
            segment_key = f"segment{idx + 1}"
            pred1 = prediction.get(segment_key, "N/A")
            pred2 = extract_answer(client, pred1, question)
...
def main():
    ...
    output_dir = f"static/GPT3.5-assisted-Vanilla-Eval/Vanilla_{score_name}"            # Output directory for annotated results
    output_json = f"static/GPT3.5-assisted-Vanilla-Eval/Vanilla_{score_name}/combined_results.json"        # File for combined results
